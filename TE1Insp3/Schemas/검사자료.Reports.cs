...
namespace TE1.Schemas.Reports
{
    public class 검사내역 : IDisposable
    {
        ...
        public void Init(모델구분 모델)
        {
            ...
            col = this.검사자료.Columns.Add(nameof(검사결과.검사번호), typeof(Int32));
            col.Caption = "Index";
            ...
            col = this.검사자료.Columns.Add(nameof(검사결과.큐알내용), typeof(String));
            col.Caption = "QR";
            col.ExtendedProperties[Band] = Item;
            ...
            foreach (검사정보 설정 in Global.모델자료.GetItem(모델구분).검사설정)
            {
                ...
                col.ExtendedProperties[Band] = GetBand(설정.검사분류);
                col.ExtendedProperties[FormatType] = DevExpress.Utils.FormatType.Numeric;
        ...
        public String GetBand(Int32 분류) => Global.분류자료.GetItem(분류)?.명칭;
            ...
            foreach (검사결과 결과 in 자료)
            {
                ...
                foreach (검사정보 검사 in 결과.검사내역)
                {
                    ...
                    if (this.검사자료.Columns.Contains(name))
                    {
                        row[name] = 검사.결과값;
                        Boolean OK = false;
                        if (DeviceInfoAttribute.IsCamera(검사.검사장치)) OK = 검사.측정결과 == 결과구분.OK;
        ...
        {
            결과 = this.결과자료.Where(e => e.검사번호 == 검사코드).FirstOrDefault();
