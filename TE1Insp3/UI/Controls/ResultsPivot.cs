...
namespace TE1.UI.Controls
{
    public partial class ResultsPivot : XtraUserControl
    {
        ...
        public void Init()
        {
            ...
            this.e시작일자.DateTime = Global.환경설정.주간시작일시();
            this.e종료일자.DateTime = Global.환경설정.야간종료일시();
            ...
            foreach(분류정보 분류 in Global.분류자료)
            {
                GridBand band = new GridBand() { Caption = 분류.명칭, VisibleIndex = this.GridView1.Bands.Count };
                band.AppearanceHeader.TextOptions.HAlignment = HorzAlignment.Center;
                band.AppearanceHeader.Options.UseTextOptions = true;
                this.GridView1.Bands.Add(band);
            }
        ...
        private void 검사결과보기(object sender, EventArgs e)
        {
            ...
            Int32 코드 = Utils.IntValue(row[nameof(검사결과.검사번호)]);
